...
function doIPQuery($product_id, $isFormatted = true)
{
	...
	while($myrow = mysql_fetch_assoc($rs)) {
		if ($isFormatted)
		{	
			print "<tr><td>" . $myrow['name'] . "</td><td>" . "<a href=\"https://bugs.eclipse.org/bugs/show_bug.cgi?id=" . $myrow['bug_id'] . "\">" . $myrow['bug_id'] . "</a>" . "</td><td>" . $myrow['login_name'] . "</td><td>" . $myrow['size'] . "</td><td>" . str_replace(",", " ", $myrow['short_desc']) . " (" . str_replace(",", " ", $myrow['description']) . ")</td></tr>\n";
		}
		else
		{
